...
function ExpenseHistory({ expenses }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 w-full text-center p-5 bg-blue-800 text-white rounded">
        <div >
          ...
          <div className="relative w-full max-w-5xl">
            ...
            <div>
              <input
                ...
                className="pl-9 pr-3 py-2 w-full rounded bg-white text-black placeholder-gray-500 focus:outline-none focus:shadow-outline"
              />
            </div>
          </div>
        </div>
          ...
          <DropdownMenu iconType="filter">
            <div className="w-48 mx-auto my-4">
              {items.map((item, index) => (
                <div
                  key={index}
                  className={`w-full p-3 mb-4 bg-gray-100 text-black rounded shadow hover:bg-gray-200 text-center ${selectedFilters.includes(item.name) ? 'bg-blue-500' : ''
                    }`}
      ...
      <div className="flex flex-col items-center justify-top p-4 text-black bg-white rounded shadow">
        <div className="w-full">
          {filteredHistory.length > 0 ? (
            ...
            <div className="text-gray-500 text-center">No expense history found.</div>
          )}
        </div>
      </div>
    </div>
